...
function apiFetch(path, options) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var timeout = setTimeout(function() {
      ...
      reject(new Error('网络超时(15秒)。URL:' + url));

    }, 15000);

    fetch(url, Object.assign({ headers: headers }, options || {}))

      .then(function(r) {

        clearTimeout(timeout);

        console.log('[API] Response status:', r.status, 'from:', url);

        if (!r.ok) {

          console.error('[API] HTTP Error:', r.status, r.statusText);
      ...
      .catch(function(e) {
        ...
        console.error('[API] Fetch Error:', e.name, '-', e.message, 'URL:', url);
        ...
        if (e.name === 'TypeError' && e.message.indexOf('Failed to fetch') >= 0) {
          ...
          errMsg = '无法连接服务器。请检查: 1)手机联网 2)在浏览器打开: ' + API_BASE;
        ...
        reject(new Error(errMsg + ' (详情:' + e.name + ')'));
...
var Sound = {
  ...
  _getCtx: function() {
    ...
    if (!this._gainNode && this._ctx) {
      ...
      this._gainNode.gain.value = (this.volume / 100) * 0.15;
...
var Music = {
  ...
  stop: function() {
    ...
    var now = this._ctx ? this._ctx.currentTime : 0;
    this._activeNodes.forEach(function(gain) {
  ...
  setVolume: function(vol) {
    ...
    // Update master gain for all active nodes
    var now = this._ctx ? this._ctx.currentTime : 0;
    var targetVol = (this.volume / 100) * 0.15;
    this._activeNodes.forEach(function(gain) {
      try { gain.gain.cancelScheduledValues(now); } catch(e) {}
      try { gain.gain.setValueAtTime(targetVol, now); } catch(e) {}
    });
    // Also update master gain node
    if (this._gainNode) {
      try { this._gainNode.gain.cancelScheduledValues(now); } catch(e) {}
      try { this._gainNode.gain.setValueAtTime(targetVol, now); } catch(e) {}
    }
  },
...
function updateUserDisplay() {
  ...
  userEls.forEach(function(el) {
    ...
    el.textContent = name || (phone ? String(phone).replace(/(\d{3})\d{4}(\d{4})/, '$1****$2') : '未登录');
...
function showLogin() {
  ...
  if (toggleBtn) toggleBtn.textContent = '还没有账号？点此注册';
...
function toggleLoginMode() {
  ...
  var isHidden = nameGroup.style.display === 'none';
  ...
  if (isRegisterMode) {
    ...
    registerUser(phone, password, name, school, birthdate)
      ...
      .catch(function(e) {
        ...
        document.getElementById('login-msg').textContent = e.message || '网络错误，请检查网络后重试';
...
function renderVocabReviewCard() {
  ...
  html += "<button class=\"flashcard-nav-btn\" onclick=\"prevVocabReviewCard()\" " + (idx === 0 ? "disabled" : "") + ">\xe2\x86\x90</button>";
  html += '<div class="flashcard-progress">' + (idx + 1) + ' / ' + total + '</div>';

  html += "<button class=\"flashcard-nav-btn\" onclick=\"nextVocabReviewCard()\" " + (idx >= total - 1 ? "disabled" : "") + ">\xe2\x86\x92</button>";
  html += '</div>';
  ...
  if (friendCurrentTab === 'list') {
    ...
    if (friends.length === 0) {
      ...
      friends.forEach(function(f) {
        ...
        var staminaCount = todaySent[f.id] || 0;
        var staminaDisabled = staminaCount >= 3 ? 'disabled style="opacity:0.5;"' : '';
        var coinsToday = Storage.getTodaySentCoins();
        var coinsDisabled = coinsToday >= 100 ? 'disabled style="opacity:0.5;"' : '';
        var friendLikes = Storage.getFriendLikes();
        var liked = friendLikes.indexOf(f.id) >= 0;
        ...
        html += '<button class="friend-btn stamina" onclick="sendStamina(\'' + f.id + '\',\'' + f.name + '\')" ' + staminaDisabled + '>体力(' + (3 - staminaCount) + ')</button>';
        html += '<button class="friend-btn coins" onclick="sendCoins(\'' + f.id + '\',\'' + f.name + '\')" ' + coinsDisabled + '>金币</button>';
        html += '<button class="friend-btn like" onclick="likeFriend(\'' + f.id + '\',\'' + f.name + '\')" ' + (liked ? 'disabled style="opacity:0.5;"' : '') + '>' + (liked ? '已赞❤️' : '点赞🤍') + '</button>';
...
function sendStamina(friendId, friendName) {
  var result = Storage.sendStaminaToFriend(friendId);
  showToast(result.message);
  if (result.success) {
    showToast('已赠送体力给 ' + friendName);
    renderFriendsContent();
  }
}

function sendCoins(friendId, friendName) {
  var amount = prompt('赠送多少金币给 ' + friendName + '？(1-50，今日上限100)', '10');
  if (!amount) return;
  amount = parseInt(amount);
  if (isNaN(amount) || amount <= 0 || amount > 50) {
    showToast('请输入1-50之间的数字');
    return;
  }
  var result = Storage.sendCoinsToFriend(friendId, amount);
  showToast(result.message);
  if (result.success) {
    showToast('已赠送 ' + amount + ' 金币给 ' + friendName);
    renderFriendsContent();
  }
}

function likeFriend(friendId, friendName) {
  var result = Storage.likeFriend(friendId);
  if (result) {
    showToast('已点赞 ' + friendName);
    renderFriendsContent();
  } else {
    showToast('今日已点赞过该好友');
  }
...
function showInteractionsHistory() {
  var interactions = Storage.getInteractions();
  var html = '<div style="padding:20px;">';
  html += '<h3>📜 互动历史</h3>';
  if (interactions.length === 0) {
    html += '<div style="text-align:center;padding:40px;color:#888;">暂无互动记录</div>';
  } else {
    interactions.forEach(function(i) {
      var time = new Date(i.timestamp).toLocaleString('zh-CN');
      var action = '';
      if (i.type === 'stamina') {
        action = '赠送体力';
      } else if (i.type === 'coins') {
        action = '赠送 ' + i.amount + ' 金币';
      } else if (i.type === 'like') {
        action = '点赞了';
      }
      html += '<div style="padding:10px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;">';
      html += '<span>' + i.fromName + ' ' + action + ' ' + i.toName + '</span>';
      html += '<span style="color:#888;font-size:0.85em;">' + time + '</span>';
      html += '</div>';
    });
  }
  html += '</div>';
  document.getElementById('interactions-history').innerHTML = html;
  showScreen('interactions-history-page');
}
